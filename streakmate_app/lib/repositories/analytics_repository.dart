...
class AnalyticsRepository {
  ...
  Future<List<HeatmapDay>> getHeatmap(int year) async {
    try {
      ...
      _check(r);
      final raw = r.data['data']['heatmap'] as Map<String, dynamic>? ?? {};
      return raw.entries
          .map((e) => HeatmapDay.fromJson(
              e.key, e.value as Map<String, dynamic>))
          .toList()
        ..sort((a, b) => a.date.compareTo(b.date));
    } on DioException catch (e) {
      debugPrint('[Analytics] heatmap error: ${e.message}');
      return []; // non-fatal — heatmap can show empty
    }
  }
